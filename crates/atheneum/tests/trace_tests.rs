...
fn test_query_trace_and_navigate_trace() {
    ...
    let (_views, trace_id) = graph
        .navigate_with_trace("Grounded Theory", 5, 2, None, None, None, true)
        .unwrap();
    ...
    let trace_entity = graph
        .with_raw_connection(|conn| {
            let mut stmt =
                conn.prepare("SELECT kind, name, data FROM graph_entities WHERE id = ?1")?;
    ...
    assert!(trace_entity
        .1
        .to_lowercase()
        .contains("trace: grounded theory"));
    ...
    let edges = graph
        .with_raw_connection(|conn| {
            let mut stmt =
                conn.prepare("SELECT to_id, edge_type FROM graph_edges WHERE from_id = ?1")?;
